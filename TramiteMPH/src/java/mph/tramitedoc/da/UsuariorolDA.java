...
try{
	...
	String sql="";
if (oUsuariorolBE1.getIndOpSp() == 1) {
	sql = " SELECT '<i style=\"cursor:pointer;\" onclick=\"edit('||idusuariorol||')\" class=\"fa fa-pencil-square-o\"></i>','<i style=\"cursor:pointer;\" onclick=\"del('||idusuariorol||')\" class=\"fa fa-trash-o\"></i>',idusuariorol,idusuario,idrol,fechaasignacion,estado FROM usuariorol WHERE estado=true";
pst = cn.prepareStatement(sql);
rs = pst.executeQuery();
}
...
cs=cn.prepareCall("{call uspEliminarUsuariorol(?)}");cs.setInt(1, oUsuariorolBE.getIdusuariorol());	cs.registerOutParameter(1, java.sql.Types.INTEGER);
	cs.executeUpdate();
...
return resultado;
 ...
 public ResultSet listarRS(String query) throws SQLException {
  ...
  public List listObjectUsuariorolBE(UsuariorolBE oUsuariorolBE) throws SQLException {
	 ...
	 sql = " SELECT idusuariorol,idusuario,idrol,fechaasignacion,estado FROM usuariorol WHERE idusuariorol=? and estado=true";                pst = cn.prepareStatement(sql);
