...
cs=cn.prepareCall("{call uspEliminarEstadoflujo(?)}");cs.setInt(1, oEstadoflujoBE.getIdestadoflujo());	cs.registerOutParameter(1, java.sql.Types.INTEGER);
	cs.executeUpdate();
...
return resultado;
 ...
 public ResultSet listarRS(String query) throws SQLException {
  ...
  public List listObjectEstadoflujoBE(EstadoflujoBE oEstadoflujoBE) throws SQLException {
	 ...
	 sql = " SELECT idestadoflujo,denominacion,estado FROM estadoflujo WHERE idestadoflujo=? and estado=true";                pst = cn.prepareStatement(sql);
