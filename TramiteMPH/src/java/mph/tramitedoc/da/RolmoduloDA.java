...
cs=cn.prepareCall("{call uspEliminarRolmodulo(?)}");cs.setInt(1, oRolmoduloBE.getIdrolmodulo());	cs.registerOutParameter(1, java.sql.Types.INTEGER);
	cs.executeUpdate();
...
return resultado;
 ...
 public ResultSet listarRS(String query) throws SQLException {
  ...
  public List listObjectRolmoduloBE(RolmoduloBE oRolmoduloBE) throws SQLException {
	 ...
	 sql = " SELECT idrolmodulo,idrol,idmodulo,fechaasignacion,estado FROM rolmodulo WHERE idrolmodulo=? and estado=true";                pst = cn.prepareStatement(sql);
