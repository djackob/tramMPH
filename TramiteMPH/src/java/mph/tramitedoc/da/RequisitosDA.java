...
cs=cn.prepareCall("{call uspEliminarRequisitos(?)}");cs.setInt(1, oRequisitosBE.getIdrequisitos());	cs.registerOutParameter(1, java.sql.Types.INTEGER);
	cs.executeUpdate();
...
return resultado;
 ...
 public ResultSet listarRS(String query) throws SQLException {
  ...
  public List listObjectRequisitosBE(RequisitosBE oRequisitosBE) throws SQLException {
	 ...
	 sql = " SELECT idrequisitos,idprocedimiento,denominacion,estado FROM requisitos WHERE idrequisitos=? and estado=true";                pst = cn.prepareStatement(sql);
