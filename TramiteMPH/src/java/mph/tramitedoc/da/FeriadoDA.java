...
try{
	...
	String sql="";
if (oFeriadoBE1.getIndOpSp() == 1) {
	sql = " SELECT '<i style=\"cursor:pointer;\" onclick=\"edit('||idferiado||')\" class=\"fa fa-pencil-square-o\"></i>','<i style=\"cursor:pointer;\" onclick=\"del('||idferiado||')\" class=\"fa fa-trash-o\"></i>',idferiado,idanio,fecha,motivo,estado FROM feriado WHERE estado=true";
pst = cn.prepareStatement(sql);
rs = pst.executeQuery();
}
...
cs=cn.prepareCall("{call uspEliminarFeriado(?)}");cs.setInt(1, oFeriadoBE.getIdferiado());	cs.registerOutParameter(1, java.sql.Types.INTEGER);
	cs.executeUpdate();
...
return resultado;
 ...
 public ResultSet listarRS(String query) throws SQLException {
  ...
  public List listObjectFeriadoBE(FeriadoBE oFeriadoBE) throws SQLException {
	 ...
	 sql = " SELECT idferiado,idanio,fecha,motivo,estado FROM feriado WHERE idferiado=? and estado=true";                pst = cn.prepareStatement(sql);
